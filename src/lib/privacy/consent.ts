...
export type ConsentRecord = Readonly<{
  /** Policy version this consent was given against. */
  v: string;
  /** Unix epoch (ms) when the user made the choice. */
  ts: number;
...
/**
 * Parse the consent cookie value. Returns null if the cookie is missing,
 * malformed, or recorded against an older policy version (the user has to
